...
void FOC::initSPI() {
    ...
    digitalWrite(CS_2, HIGH);   //sensor is of the low - active type
    SPI.setSCK(CLK);
...
void FOC::SPITransfer() {
    ...
    SPI.beginTransaction(spiSettings);  // clock: max. 10MHz | MSB first | SPI Mode 1  | CPOL=0, CPHA= 1
    digitalWriteFast(CS_2, LOW); //SPI Mode 1 -> information gest sampled with falling endge
    uint16_t angle_2 = SPI.transfer16(0xFFFF) & 0x3FFFu;
    //command bit15 = 1 (parity) | bit14 = 1 (read) | adresss 0x000 -> B11000000 00000000 = 0xC0000;
    digitalWriteFast(CS_2, HIGH); //SPI Mode 1 -> receive information with rising edge
    digitalWriteFast(CS, LOW);
    uint16_t angle = SPI.transfer16(0xFFFF) & 0x3FFFu;
    ...
    rotor_position_2 = fast_mod(angle_2, 1489) * 0.24169f;
...
void FOC::initInhibitPIN() {
    ...
    pinMode(INH_W_2, OUTPUT);
    pinMode(INH_U_2, OUTPUT);
    pinMode(INH_V_2, OUTPUT);
}
...
void FOC::activateInhibitPins() {
    ...
    digitalWriteFast(INH_W_2, HIGH);
    digitalWriteFast(INH_U_2, HIGH);
    digitalWriteFast(INH_V_2, HIGH);
...
void FOC::setInputPinsDutyCycle() {
    ...
    FTM0_C1V = IN_W_2_duty_cycle; //Teency pin 23 -> FTM0_CH1
    FTM0_C6V = IN_U_2_duty_cycle; //Teency pin 21 (A8) -> FTM0_CH6
    FTM0_C0V = IN_V_2_duty_cycle; //Teency pin 22 (A9) -> FTM0_CH0


    FTM0_C3V = IN_W_duty_cycle; //Teency pin 10 -> FTM0_CH3
    FTM0_C7V = IN_U_duty_cycle; //Teency pin 5 -> FTM0_CH7
    FTM0_C4V = IN_V_duty_cycle; //Teency pin  6 -> FTM0_CH4
...
void FOC::doTheMagic() {
    ...
    if(control_counter>2000){
        ...
        //Serial.println(rotor_position_2);

        //calculateRPS();
        Serial.println(rotor_position);
        //target_speed = 90;
        //target_speed_2 = 80;
        //P_controller(4.0f,my_speed);
        //P_controller_2(target_rps,my_speed_2);
        /*
...
void FOC::initPins() {
    ...
    FOC::calculate_offset_parameters_short(); // done
    initSPI(); // done
    initInhibitPIN(); // done
    setPWMpins(); // done

    initServoPins(); // done
    activateInhibitPins(); // done
    //initADCconversions(); // done
    //calculateSensorOffsetAngle();
...
void FOC::calculateInputPinsDutyCycleLUT() {
    ...
    IN_W_2_duty_cycle =
            array[fast_mod(rotor_position_2 + sensor_offset_2 + angle_offset * direction_2, 360)] * (target_speed_2 * 0.01f) +
            modulo_index_offset;
    IN_V_2_duty_cycle = array[fast_mod(rotor_position_2 + sensor_offset_2 + angle_offset * direction_2 + 120, 360)] *
                      (target_speed_2 * 0.01f) + modulo_index_offset;
    IN_U_2_duty_cycle = array[fast_mod(rotor_position_2 + sensor_offset_2 + angle_offset * direction_2 + 240, 360)] *
                      (target_speed_2 * 0.01f) + modulo_index_offset;
...
//FTM0_MODE|=0x05; // 0000 0101
    ...
    FTM0_C0V = 0; //50%
    PORTC_PCR1 |= PORT_PCR_MUX(4) | PORT_PCR_DSE | PORT_PCR_SRE; //Teensy pin 22 -> FTM0_CH0
...
void FOC::P_controller_2(float rps_target, float rps_measured) {
...
void FOC::calculateSensorOffsetAngle() {
    ...
    uint16_t my_psudo_rotor_posion = 10;
...
void FOC::calculateRPS() {

    {
        ...
        static int16_t angle_before_2 = 0;
        int16_t angle_now_local=angle_now;
        int16_t angle_now_local_2=angle_now_2;
        ...
        if (sensor_overflow_2 == 1) {
            angle_now_local_2 += 16384 * direction_2;
            sensor_overflow_2 = 0;
        }
        ...
        my_speed_2 = ((angle_now_local_2 - angle_before_2) / 16384.0f) * 10; // 10 cuz are doing this 10 time per sec
        angle_before_2 = angle_now_2;
