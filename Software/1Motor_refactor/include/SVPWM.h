...
public:
    ...
    static constexpr uint16_t calculateDutyCycleW(uint16_t rotor_position,float modulation_index,uint16_t MAX_DUTY_CYCLE);
...
constexpr std::array<uint16_t, LUTGenerator::LUTSize> LUTGenerator::generate() {
...
constexpr uint16_t LUTGenerator::calculateDutyCycleW(uint16_t rotor_position,float modulation_index,uint16_t MAX_DUTY_CYCLE){
...
constexpr ModulationIndexScalingParams LUTGenerator::calculateModulationIndexScalingOffsetParameters() {
...
private:
    ...
    static constexpr uint16_t angleOffset = LUTSize/4; // +90°, -90°
...
public:
    ...
    inline static SPWMDutyCycles calculateDutyCycles(Motor &x){ //@TODO should return a const ref as every non trivial return value should
        SPWMDutyCycles temp;
       float modulationIndexOffset =  scaleDutyCyclesToModulationIndex(x.speedScalar);

       //@TODO : add direction instead of field weakening
